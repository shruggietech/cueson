...
The shared `scripted-*` matrix rows map to the single ASS/SSA contract and require both dialects rather than duplicate row identifiers. Evidence kind `deferred` identifies applicable future assertions and names their owning issues. S026 replaces applicable conversion deferrals with executable conversion evidence; S028 removes the final stable deferrals after genuine stable-identity proof. S027 completes platform/hostile-input hardening; S028 closes the stable freeze and exact candidate render/platform proof. Public release verification remains #66. `inapplicable` remains reserved for assertions that do not apply to the row.

S026 adds six independently authored synthetic conversion fixture records with 28 artifacts under `fixtures/scripted-conversion/`. Four shared baseline sources each own three exact target-byte oracles and one semantic/ordered-loss expectation, explicitly covering all twelve distinct SubRip/WebVTT/ASS/SSA directions. Two paired native-loss sources each own two text-target oracles and a complete ordered expectation (40 atomic ASS losses and 36 SSA losses). These source bytes, target bytes, and structural loss pointers were prescribed from the conversion contract rather than generated through an update-goldens mode. Every record has explicit MIT redistribution approval and does not modify S025 native capture, projection, diagnostic, or render expectations.

Conversion expectations expose reparsed start/end milliseconds, plain text, logical lines, and the complete ordered loss-code/source-pointer list. Cross-package tests additionally validate each full report against its immutable source, repeated byte/report determinism, native/Cue JSON parity, source integrity and historical identity, strict complete-report refusal, and fatal refusal through default stdout, explicit stdout, new destination, and forced existing destination. Focused cases check exact and neighboring centisecond boundaries, ties upward, interval collapse, literal control/braces refusal, zero/empty/drawing-only text refusal, preserved malformed owners, retained variant declarations/unknown fields/attachments, and caller-path privacy. Conversion is model-derived output; its oracle never substitutes for the source-byte restoration oracle.
