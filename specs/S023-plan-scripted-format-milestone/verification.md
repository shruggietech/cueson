...
The single second review identified a privacy gap for unknown authoring metadata. The accepted-source restriction now covers known and unknown metadata keys/sections; path/resource/identity-bearing and unclassifiable authoring metadata rejects the whole operation without sanitization or publication. Explicit dialogue, font-family, and bounded embedded-content roles retain their content semantics. The acceptance matrix requires hostile unknown-metadata and exempt-content evidence. This deliberately narrows accepted unknown authoring metadata instead of risking local information in retained raw records or original-source bytes. No third review is requested; the finding is replied to and resolved after remediation, and current-head CI plus policy reconciliation remains required.
