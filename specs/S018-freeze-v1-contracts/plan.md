...
**Performance Goals**: Reject native inputs above 64 MiB, reject encode output and Cue JSON restore or render input above 1 GiB, cap adversarial document collections, per-item occurrences, diagnostics, and loss observations at explicit deterministic limits; run every required fuzz surface at a fixed bounded work count in hosted CI; keep ordinary valid subtitle workflows within existing test latency
