...
Round-one review clarified style emphasis accounting: track bold, italic, and underline separately against readable runs for the current initial/reset style, accumulated across cues. Mere reference does not represent a value cancelled before all readable text; both nonzero and neutral overridden values require atomic style-field omission when no matching readable run survives. This refines FR-004 without changing baseline or legacy code meanings.
