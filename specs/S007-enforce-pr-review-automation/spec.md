...
- **FR-013**: Once a second-round request exists, the policy MUST NOT generate a third automated review request under any later state; an operator-issued request MUST cite exactly one backticked commit prefix that uniquely resolves to a collected pull-request head before its result can be attributed, while an unbound request still consumes the allowance and fails closed.
