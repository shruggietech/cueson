...
- [x] T041 [Hosted] Treat only the target-derived Syft catalog roots for `cueson` and `cueson.exe` as virtual artifact identities while continuing to reject all other structurally absolute metadata paths in `scripts/release-verify/verify.go`
