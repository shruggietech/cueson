...
- [x] T049 Clear every remediable red pull-request surface through trusted workflow execution, record any operator-only recovery decision explicitly, re-audit reviews and hosted checks, and stop only when the GitHub pull request is visibly ready or a concrete human decision is required, per FR-024 (partial)
