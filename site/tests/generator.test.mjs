...
test("deployment workflow is manual-only and requires the exact current main revision", async () => {
  ...
  assert.match(workflow, /test\s+"\$GITHUB_REF"\s+=\s+"refs\/heads\/main"/);
  assert.match(workflow, /test\s+"\$GITHUB_SHA"\s+=\s+"\$REVISION"/);
  ...
  assert.doesNotMatch(workflow, /verify:(?:cloudflare|production) -- --/);
  assert.match(workflow, /environment:\s*\n\s*name: production\s*\n\s*url: https:\/\/cueson\.io\/deployment\.json/);
  assert.doesNotMatch(workflow, /^\s{6}CLOUDFLARE_API_TOKEN:/m);
  assert.equal(workflow.match(/^\s{10}CLOUDFLARE_API_TOKEN:\s*\$\{\{ secrets\.CLOUDFLARE_API_TOKEN \}\}/gm)?.length, 3);
