...
func TestRunVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if stdout != "1.1.0\n" {
				t.Errorf("Run() stdout = %q, want %q", stdout, "1.1.0\\n")
			}
			if stderr != "" {
...
func TestRunSchemaVersion(t *testing.T) {
	...
	for _, args := range [][]string{{"schema", "--version"}, {"--quiet", "schema", "--version"}} {
		status, stdout, stderr := runForTest(context.Background(), args)
		if status != ExitSuccess || stdout != "1.1.0\n" || stderr != "" {
			t.Errorf("Run(%q) = (%d, %q, %q), want (0, %q, empty)", args, status, stdout, stderr, "1.1.0\\n")
		}
	}
}
...
func TestEncodeRenderAndRestoreWorkflow(t *testing.T) {
	...
	if document.Schema != schema.ID() || document.SchemaVersion != "1.1.0" || document.Producer.Name != "cueson" || document.Producer.Version != "1.1.0" {
