...
**Status:** Published and independently verified 1.1.0 architecture; historical releases unchanged

**Ratified:** 2026-09-09 through Spec Kit slice `001-ratify-foundation-contracts`

This document is the architecture of record for the current published 1.1.0 stable release, retaining the published v1.0.0 baseline. The [project constitution](../.specify/memory/constitution.md) remains the highest repository authority. The [working project specification](Cueson-Project-Specification-v0.0.0.md) supplies broader context and roadmap detail when it does not conflict with ratified slices.
...
S028 promoted the frozen bounded four-format contract to exact 1.1.0 and prepared its immutable repository schema. Schema-only and complete experimental current-input tuples remain truthful accepted observations; official new native output is stable. The same six-target bundle was structurally accepted and its matching packaged binary executed native/historical/safety workflows on governed amd64 hosts, while a verified published v1.0.0 consumer rejected fresh current output. S029 bound post-squash main revision `7ff45c1d8cd8df377e1fb568b9785286b649fd7c`, and #66 published and independently verified the exact tag and thirteen release assets. S030 child #76 prepares the route and site artifact; #67 governs exact-main production deployment and read-back.
...
Production deployment is an owner-controlled post-merge continuation. The manual workflow must execute from `refs/heads/main`, and its execution revision, selected full commit, checkout, generated deployment record and freshly fetched `origin/main` must all identify the same exact revision before any checkout-controlled package tooling runs. The workflow repeats the fresh equality check immediately before proof, and the complete suite must pass before Wrangler deploys. Immediately before mutation, the workflow reads the exact zone, DNS, Worker, Custom Domain and redirect state and stops on target conflicts, then fetches `origin/main` again and rejects a changed head. The protected Cloudflare token is available only to preflight, deployment and post-deployment read-back steps. Completion requires an authenticated Cloudflare read-back proving both declared bindings and preservation of unrelated state, followed by independent DNS, trusted TLS, apex, `www`, documentation, media guide, matching GitHub and public deployment revisions, release-download, immutable-schema-byte, and missing-alias probes.
