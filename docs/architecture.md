...
Production deployment is an owner-controlled post-merge continuation. A manually selected full commit must equal freshly fetched `origin/main` immediately after checkout and before any checkout-controlled package tooling runs. The workflow repeats that equality check immediately before proof; the checkout and generated deployment record must identify that exact revision, and the complete suite must pass before Wrangler deploys. Immediately before mutation, the workflow reads the exact zone, DNS, Worker, Custom Domain and redirect state and stops on target conflicts, then fetches `origin/main` again and rejects a changed head. Completion requires an authenticated Cloudflare read-back proving both declared bindings and preservation of unrelated state, followed by independent DNS, trusted TLS, apex, `www`, documentation, media guide, deployment revision, release-download, immutable-schema-byte, and missing-alias probes.
