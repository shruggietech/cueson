...
Cloudflare Workers Static Assets publishes the artifact through one `cueson-site` Worker. The apex serves the reviewed assets and Worker response policy; `www` returns 308 to the same apex path and query. Versioned schemas receive immutable caching and their schema media type, deployment metadata is not cached, HTML revalidates, and every apex response receives the declared security headers. Pull requests have no production secret and perform only build, browser, artifact, Worker, and Wrangler dry-run verification.

Production deployment is an owner-controlled post-merge continuation. A manually selected full commit must be an ancestor of `origin/main`, the checkout and generated deployment record must identify that exact revision, and the complete suite must pass before Wrangler deploys. Completion requires Cloudflare state read-back plus independent DNS, trusted TLS, apex, `www`, documentation, media guide, deployment revision, release-download, immutable-schema-byte, and missing-alias probes.
