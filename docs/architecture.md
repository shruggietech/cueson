...
**Status:** Frozen 1.1.0 stable candidate architecture; historical releases unchanged

**Ratified:** 2026-09-09 through Spec Kit slice `001-ratify-foundation-contracts`

This document is the architecture of record for the current 1.1.0 stable candidate, retaining the published v1.0.0 baseline. The [project constitution](../.specify/memory/constitution.md) remains the highest repository authority. The [working project specification](Cueson-Project-Specification-v0.0.0.md) supplies broader context and roadmap detail when it does not conflict with ratified slices.

## Current candidate architecture

Current source stages executable and canonical/immutable candidate schema identity `1.1.0` together. The local schema registry selects an exact supported current or historical 1.0.0 URI/version pair before structural and version-specific semantic validation. Current discovery and all new native output use the candidate contract; historical input preserves producer, source truth and loaded identity. Unknown, mismatched and former development identities never authorize network retrieval. Cue JSON command paths complete integrity validation before output publication.
...
S028 returns package proof to stable immutable-copy verification at exact 1.1.0 after the S024-S027 development work. Released schemas and historical evidence remain immutable; reviewed candidate and post-squash main evidence must bind their own exact source revisions.
...
Current source implements `internal/codec` as a capability registry plus bounded detection/decoding and native SubRip, WebVTT and bounded stable ASS/SSA parser/renderers. `internal/convert` owns all twelve distinct four-format projections, compatibility analysis, complete deterministic runtime-only loss reports, and source-aware conversion rendering. Scripted targets use the stable bounded profile; `internal/ocr` remains reserved future ownership.
...
The common model exposes timing and semantic content without requiring consumers to decode source bytes or parse a subtitle grammar. Format-native data remains adjacent wherever normalization would otherwise lose information. Current SubRip, WebVTT and bounded stable ASS/SSA encode workflows construct both views from one bounded exact source acquisition.
...
The runnable artifact and public-byte evidence remain in [release verification](release-verification.md). The broader [release process](release-process.md) separates candidate preparation and authorized GitHub publication from milestone closure, signatures, attestations, and production actions.

S028 promotes the frozen bounded four-format contract to exact 1.1.0 and prepares an immutable repository candidate schema. Schema-only and complete experimental current-input tuples remain truthful accepted observations; official new native output is stable. The same six-target bundle is structurally accepted and its matching packaged binary executes native/historical/safety workflows on governed amd64 hosts, while a verified published v1.0.0 consumer rejects fresh current output. Pull-request proof binds only its exact head; post-squash main proof must establish the later publication target. No tag, public release, schema route activation or deployment occurs in this candidate slice.

## Public site and schema delivery

The `site/` package produces the static `cueson.io` product and documentation artifact. Root repository Markdown, immutable schemas under `schema/releases/`, and selected byte-verified files from the approved Cueson brand kit remain authoritative. `site/content-map.json` is the single route and copy allowlist. Its bounded generator rejects unsafe paths and collisions, validates source encoding and fixed schema or brand hashes, rewrites maintained documentation links, and writes disposable MDX, public copies, a content manifest, and a deployment record. Generated inputs and build output are ignored and must reproduce exactly from a clean checkout.

Next.js static export and Fumadocs render the generated documentation without becoming a second documentation authority. The artifact contains the branded landing page, complete maintained documentation, the authoritative media-format guide, and only immutable versioned schema paths. There is deliberately no `latest` schema alias. Browser verification covers every planned route, internal navigation, metadata, keyboard focus, serious or critical accessibility findings, and 360, 768, and 1440 CSS-pixel viewports.

Cloudflare Workers Static Assets publishes the artifact through one `cueson-site` Worker. The apex serves the reviewed assets and Worker response policy; `www` returns 308 to the same apex path and query. Successful versioned schemas and content-addressed framework assets receive immutable caching, stable brand paths revalidate, unsuccessful responses and deployment metadata are not cached, HTML revalidates, and every apex response receives the declared security headers. Pull requests have no production secret and perform only build, browser, artifact, Worker, and Wrangler dry-run verification.

Production deployment is an owner-controlled post-merge continuation. A manually selected full commit must be an ancestor of `origin/main`, the checkout and generated deployment record must identify that exact revision, and the complete suite must pass before Wrangler deploys. Immediately before mutation, the deployment reads the exact zone, DNS, Worker, Custom Domain, and redirect state and stops on target conflicts. Completion requires an authenticated Cloudflare read-back proving both declared bindings and preservation of unrelated state, followed by independent DNS, trusted TLS, apex, `www`, documentation, media guide, deployment revision, release-download, immutable-schema-byte, and missing-alias probes.

Public DNS verification queries A and AAAA independently through both the system resolver and the configured DNS-over-HTTPS resolver. Each hostname must supply at least one usable address family through each resolver; IPv4-only, IPv6-only, and dual-stack deployments are accepted. DNS-over-HTTPS acceptance requires successful HTTP/DNS status and correctly typed valid address records, so aliases alone cannot establish resolution. A failed sibling family does not suppress usable addresses, and complete failure reports both family outcomes in a deterministic order.
...
The SubRip, WebVTT, and cross-format contracts frozen by S018 carry stable published identity 1.0.0 through S019 and S020. S021 adds the separately authorized public site and immutable schema-hosting architecture without changing the released software or schema bytes. Signatures and attestations remain outside this slice.
