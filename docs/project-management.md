...
The `v0.0.0` milestone is closed after its release and repository reconciliation. In the open `v1.0.0` milestone, implementation and candidate issues #30 through #37 and #41 are closed; epic [#29](https://github.com/shruggietech/cueson/issues/29) and publication issue [#38](https://github.com/shruggietech/cueson/issues/38) remain open pending the S020 merge and separately authorized lifecycle reconciliation. Issue #38 appears once in `cueson Delivery` with Slice `S020`, Stage `PR review`, and an unused default Status field while its verified-publication record proceeds through review.
