...
Work slices S001 through S019 are present on `main`. The resulting repository includes the ratified foundation, published v0.0.0 envelope release, native SubRip and WebVTT workflows, conversion, validation, inspection, shell completion, bounded whole-system conformance, annotated schema, executable documentation, and the exact verified v1.0.0 candidate. S020 has published annotated tag [`v1.0.0`](https://github.com/shruggietech/cueson/tree/v1.0.0) and the independently verified [GitHub Release](https://github.com/shruggietech/cueson/releases/tag/v1.0.0) through issue #38; repository reconciliation remains in progress until its reviewed pull request merges.
...
The `v0.0.0` milestone is closed after its release and repository reconciliation. In the open `v1.0.0` milestone, implementation and candidate issues #30 through #37 and #41 are closed; epic [#29](https://github.com/shruggietech/cueson/issues/29) and publication issue [#38](https://github.com/shruggietech/cueson/issues/38) remain open pending the S020 merge and separately authorized lifecycle reconciliation. Issue #38 appears once in `cueson Delivery` with Slice `S020`, Stage `Release verification`, and an unused default Status field while its verified-publication record proceeds through review.

The v1 release binds version and schema lockstep, stable SubRip and WebVTT declarations, immutable schema bytes, concise release notes, six archives, six target-bound SPDX JSON SBOMs, and one checksum manifest to exact revision `2cad4c816340404289b4d1d87179a4071713bb46`. S020 used explicit authority for tag and GitHub Release publication. Pull-request merge, v1 epic and milestone closure, public schema hosting, signatures, attestations, and production-domain activation remain separate protected transitions.
