...
The ShruggieTech security owner provisions `CLOUDFLARE_API_TOKEN` as a secret in the GitHub `production` environment. The account-owned token is restricted to the ShruggieTech Cloudflare account and the `cueson.io` zone. Its permissions are Account Workers Scripts Write plus Zone Zone Read, DNS Read, and Transform Rules Read. Workers Scripts Write covers the `cueson-site` upload and Custom Domain operations; the read permissions cover exact zone identity, DNS, Worker inventory, Custom Domains, and redirect rulesets used by preflight and read-back. The token is exposed only to the Cloudflare preflight, Wrangler deployment, and Cloudflare post-deployment read-back steps. Checkout, setup, dependency installation, tests, local artifact proof, and public verification do not receive it.
