...
S023 ratified the next compatible v1.1.0 scripted-format milestone. S024 staged current `1.1.0-dev`, exact historical 1.0.0 input compatibility and ASS/SSA schema-only models. S025 adds provenance-governed corpus evidence and experimental native ingest/render capabilities while retaining schema-only input observations. S026 completes the twelve-direction conversion graph; S027 completes scripted CLI discovery/inspection and development conformance hardening. S028 freezes the bounded stable ASS/SSA profile and promotes the exact current software/schema candidate to 1.1.0 with immutable-copy, historical-input, six-target package and published-old-consumer refusal proof. Separately authorized official publication and public hosting remain subsequent gates. The [development roadmap](roadmap.md) owns chronological slice grouping and native GitHub issue dependencies; draft format section order is not an execution priority.
