...
S023 ratified the next compatible v1.1.0 scripted-format milestone. S024 staged current `1.1.0-dev`, exact historical 1.0.0 input compatibility and ASS/SSA schema-only models. S025 adds provenance-governed corpus evidence and experimental native ingest/render capabilities while retaining schema-only input observations. S026 completes the twelve-direction conversion graph; S027 completes scripted CLI discovery/inspection and development conformance hardening. Stable candidate proof, separately authorized publication and public hosting remain subsequent gates. The [development roadmap](roadmap.md) owns chronological slice grouping and native GitHub issue dependencies; draft format section order is not an execution priority.
