...
These framework-light React components carry the Cueson interface grammar for mocks and
non-React surfaces. Import `../styles.css` and `components.css`, then compose the JSX
modules. Every visual value resolves through the `--cu-*` token namespace, so there is
